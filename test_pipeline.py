#!/usr/bin/env python3
"""
Test script to run the automation pipeline without git functionality.
"""
...
class TestPipeline:
    ...
    def run(self):
        """Run the test pipeline without git"""
        logger.info("Starting test automation pipeline")
        
        start_time = time.time()
        
        try:
            # Step 1: Scrape content
            logger.info("Step 1/4: Scraping content from sources")
            articles = self._scrape_content()
            logger.info(f"Scraped {len(articles)} articles")
            
            # Step 2: Classify content
            logger.info("Step 2/4: Classifying articles")
            classified_articles = self._classify_articles(articles)
            logger.info(f"Classified {len(classified_articles)} articles")
            
            # Step 3: Assess quality
            logger.info("Step 3/4: Assessing article quality")
            ...
            logger.info("Step 4/4: Generating markdown files")
...
def main():
    ...
    if not success:
        logger.error("Test pipeline completed with errors")
        return 1
